            ...
            if (ImGui::BeginTabItem("Auto-Fix Rules")) {
                ...
                if (ImGui::Button("Add")) {
                    if (strlen(newKw) > 0) {
                        ...
                        // 入力欄クリア
                        memset(newKw, 0, 128);
                        // ★追加時に即保存（明示的な Save 前にアプリが落ちても失われないように）。
                ...
                for (int i = 0; i < g_KeywordList.size(); ++i) {
                    ...
                    if (ImGui::IsItemClicked(1)) {
                        if (formHasData) {
                            ...
                            // ★item は erase で無効化されるため、先に名前をコピーしてからログに使う（use-after-erase 回避）。
                            std::string movedName = item.keyword;
                            g_KeywordList.erase(g_KeywordList.begin() + i);
                            i--;
                            SaveKeywordsJSON(); // ★編集（フォーム移動＝削除）時に即保存
                            AddLog("Moved to form: " + movedName, LogType::Info);
                        }
                    }
